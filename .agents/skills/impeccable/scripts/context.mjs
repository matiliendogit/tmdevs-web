...
function parseUpdateHost(envValue) {
  const DEFAULT = 'https://impeccable.style';
  if (!envValue?.trim()) return DEFAULT;
  try {
    const u = new URL(envValue.trim());
    if (u.hostname !== 'impeccable.style') return DEFAULT;
    return u.origin;
  } catch { return DEFAULT; }
}
const UPDATE_HOST = parseUpdateHost(process.env.IMPECCABLE_UPDATE_HOST);
...
export function resolveContextDir(cwd = process.cwd()) {
  ...
  if (envDir && envDir.trim()) {
    ...
    if (path.isAbsolute(trimmed)) {
      // Silently ignore absolute paths — only project-relative dirs are accepted.
    } else {
